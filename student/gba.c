...
void drawBox(int x, int y, volatile u16 color) {
    drawRectDMA(x, y, 10, 10, color);
}
void drawPaddle(int x, int y, u16 color) {
    drawRectDMA(x, y, 30, 5, color);
}

void drawBall(int x, int y, volatile u16 color) {
    drawRectDMA(x, y, 5, 5, color);
}

void unDrawBall(int x, int y, const u16 *image) {
    for (int r = 0; r < 5; r++) {
        DMA[DMA_CHANNEL_3].src = &image[OFFSET(x + r, y, GAMESTATE1_WIDTH)];
        DMA[DMA_CHANNEL_3].dst = &videoBuffer[OFFSET(x + r, y, GAMESTATE1_WIDTH)];
        DMA[DMA_CHANNEL_3].cnt = (5) | DMA_ON | DMA_16
                                 | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;
    }
}
void unDrawPaddle(int x, int y, const u16 *image) {
    for (int r = 0; r < 5; r++) {
        DMA[DMA_CHANNEL_3].src = &image[OFFSET(x + r, y, GAMESTATE1_WIDTH)];
        DMA[DMA_CHANNEL_3].dst = &videoBuffer[OFFSET(x + r, y, GAMESTATE1_WIDTH)];
        DMA[DMA_CHANNEL_3].cnt = 30 | DMA_ON | DMA_16
                                 | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;
    }
}

void unDrawScore(int x, int y, const u16 *image) {
    for (int r = 0; r < 10; r++) {
        DMA[DMA_CHANNEL_3].src = &image[OFFSET(x + r, y, GAMESTATE1_WIDTH)];
        DMA[DMA_CHANNEL_3].dst = &videoBuffer[OFFSET(x + r, y, GAMESTATE1_WIDTH)];
        DMA[DMA_CHANNEL_3].cnt = 20 | DMA_ON | DMA_16
                                 | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;
    }
}

void unDrawString(int x, int y, int width, int height, const u16 *image) {
    for (int r = 0; r < height; r++) {
        DMA[DMA_CHANNEL_3].src = &image[OFFSET(x + r, y, GAMESTATE1_WIDTH)];
        DMA[DMA_CHANNEL_3].dst = &videoBuffer[OFFSET(x + r, y, GAMESTATE1_WIDTH)];
        DMA[DMA_CHANNEL_3].cnt = width | DMA_ON | DMA_16
...
void unDrawBrick(int x, int y, int width, int height, const u16 *image) {
    for (int r = 0; r < height; r++) {
        DMA[DMA_CHANNEL_3].src = &image[OFFSET(x + r, y, GAMESTATE1_WIDTH)];
        DMA[DMA_CHANNEL_3].dst = &videoBuffer[OFFSET(x + r, y, GAMESTATE1_WIDTH)];
        DMA[DMA_CHANNEL_3].cnt = width | DMA_ON | DMA_16
...
void unDrawHeart(int x, int y, const u16 *image) {
    for (int r = 0; r < 10; r++) {
        DMA[DMA_CHANNEL_3].src = &image[OFFSET(x + r, y, GAMESTATE1_WIDTH)];
        DMA[DMA_CHANNEL_3].dst = &videoBuffer[OFFSET(x + r, y, GAMESTATE1_WIDTH)];
        DMA[DMA_CHANNEL_3].cnt = 50 | DMA_ON | DMA_16
                                 | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;
    }
}
